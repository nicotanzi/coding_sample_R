...
Next segment initialize some variables, and defines what is going to look for:
```{r two, eval = FALSE}
...
The main loop, iterates over every month of every year as it was previously defined.<br>
We have to be careful because the excel files are not fully consistent, for example: most of them are '.xls' but few of them are '.xlsx'; the pattern of sheet names changes; the order of sheets could also change, etc.
```{r three, eval = FALSE}
...
Finally, we transform those vector into time series
```{r four, eval = FALSE}
...
Very handy. It's syntax could be used in other contexts.